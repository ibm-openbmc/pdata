...
if ( !( $cfgSrcOutputDir =~ "none" ) )
{
    ...
    if ($buildBmc)
    {
        ...
        my $bmcfapi2PlatAttrMacrosHeaderFile = *FAPI2_PLAT_ATTR_MACROS_FILE;
        writebmcFapi2PlatAttrMacrosHeaderFileHeader($bmcfapi2PlatAttrMacrosHeaderFile);
        writebmcFapi2PlatAttrMacrosHeaderFileContent( $attributes, $fapiAttributes, $bmcfapi2PlatAttrMacrosHeaderFile );
        writebmcFapi2PlatAttrMacrosHeaderFileFooter($bmcfapi2PlatAttrMacrosHeaderFile);
        close $bmcfapi2PlatAttrMacrosHeaderFile;
    }
    else
    {
        open( FAPI2_PLAT_ATTR_MACROS_FILE, ">$cfgSrcOutputDir" . "fapi2platattrmacros.H" )
...
# BMC FAPI2 ATTRIBUTE SUPPORT
################################################################################
# Writes the BMC FAPI2 plat attribute macros header file header
################################################################################
sub writebmcFapi2PlatAttrMacrosHeaderFileHeader
{
    my ($outFile) = @_;
 ...
 *  \@file fapi2platattrmacros.H
 *
 *  \@brief FAPI2 -> plat attribute mappings.  This file is autogenerated and
 *      should not be altered.
 */
...
namespace fapiToTargeting
{
// Helper template: maps fapi2::AttributeId → TARGETING::ATTRIBUTE_ID
template <fapi2::AttributeId FapiId>
struct Fapi2ToTargetingAttr;

// Register each mapping
#define REGISTER_FAPI2_TO_TARGETING(FAPI2_ENUM, TARGETING_ENUM) \
    template <> struct Fapi2ToTargetingAttr<fapi2::FAPI2_ENUM> { \
        static constexpr TARGETING::ATTRIBUTE_ID value = TARGETING_ENUM; \
    };
...
sub writebmcFapi2PlatAttrMacrosHeaderFileContent
{
    my ( $attributes, $fapiAttributes, $outFile ) = @_;
    ...
    foreach my $attribute ( @{ $attributes->{attribute} } )
    {
        foreach my $hwpfToHbAttrMap ( @{ $attribute->{hwpfToHbAttrMap} } )
        {
            ...
            if ($instantiated)
            {
                $attrSection .= "REGISTER_FAPI2_TO_TARGETING(". $hwpfToHbAttrMap->{id} . ", " . "TARGETING::ATTR_" . $attribute->{id} . ")\n";
            }
        }
    }
...
sub writebmcFapi2PlatAttrMacrosHeaderFileFooter
{
    my ($outFile) = @_;

    print $outFile <<VERBATIM;

#endif // FAPI2_FAPIPLATATTRMACROS_H
